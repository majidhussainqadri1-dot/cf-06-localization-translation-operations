...
$t->test('Assignment revocation clears stale unit role identity',function()use($project,$state):void{
    TestHarness::assertTrue(str_contains($project,'$unitChanges[$column]=null'));
    TestHarness::assertTrue(str_contains($project,"['status']='new'"));
...
$t->test('Terminology activation preserves the approving reviewer',function()use($terms):void{
    ...
    TestHarness::assertTrue(str_contains($terms,"['reviewer_id']=get_current_user_id()"));
});
$t->test('Style guide activation preserves the original approver',function()use($terms):void{
    ...
    TestHarness::assertTrue(str_contains($terms,"['approved_by']=get_current_user_id()"));
});
