...
$t->test('Package builder produces manifest checksum and SBOM',function()use($root):void{$s=file_get_contents($root.'/tools/build-release.py');foreach(['MANIFEST.json','SHA256SUMS','SBOM.cdx.json'] as $n){TestHarness::assertTrue(str_contains($s,$n),$n);}});
