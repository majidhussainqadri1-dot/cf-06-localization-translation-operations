...
use Sabri\Localization\Domain\Translation\MessageFormatValidator;
...
$t->test('ICU plural branches and nested arguments remain structurally equivalent', function (): void {
    $source = '{count, plural, one {{name} has one lesson} other {{name} has # lessons}}';
    $target = '{count, plural, one {{name} کا ایک سبق ہے} other {{name} کے # اسباق ہیں}}';
    $schema = PlaceholderValidator::normalizeSchema(array('count' => 'integer', 'name' => 'string'));
    PlaceholderValidator::assertTarget($source, $target, $schema);
    $analysis = MessageFormatValidator::analyze($target);
    TestHarness::assertTrue(isset($analysis['selectors']['count']));
});
$t->test('ICU plural category loss is rejected', function (): void {
    TestHarness::assertThrows(
        fn () => MessageFormatValidator::assertEquivalent('{count, plural, one {One} other {Many}}', '{count, plural, other {کئی}}'),
        InvalidArgumentException::class
    );
});
$t->test('ICU selector without other branch is rejected', function (): void {
    TestHarness::assertThrows(fn () => MessageFormatValidator::analyze('{state, select, ready {Ready}}'), InvalidArgumentException::class);
});
...
$t->test('Bidi override rejected', fn () => TestHarness::assertThrows(fn () => BidiValidator::assertSafe("safe\u{202E}evil", true), InvalidArgumentException::class));
$t->test('Balanced bidi isolate is allowed for explicit mixed-direction content', fn () => BidiValidator::assertSafe("لنک \u{2066}https://example.test/abc\u{2069}", true));
$t->test('Unpaired bidi isolate is rejected', fn () => TestHarness::assertThrows(fn () => BidiValidator::assertSafe("لنک \u{2066}https://example.test/abc", true), InvalidArgumentException::class));
...
$t->test('Only low-risk C1 material is eligible for external MT', function (): void {
    TestHarness::assertTrue(RiskPolicy::machineTranslationAllowed('low', 'C1', 'platform', false));
    TestHarness::assertTrue(! RiskPolicy::machineTranslationAllowed('low', 'C2', 'platform', true));
    TestHarness::assertTrue(! RiskPolicy::machineTranslationAllowed('medium', 'C1', 'platform', true));
    TestHarness::assertTrue(! RiskPolicy::machineTranslationAllowed('high', 'C1', 'medical', true));
    TestHarness::assertTrue(! RiskPolicy::machineTranslationAllowed('low', 'C1', 'identity', true));
});
$t->test('High-risk domains always require qualified domain review', function (): void {
    TestHarness::assertTrue(RiskPolicy::requiresDomainReview('normal', 'shariah'));
    TestHarness::assertTrue(RiskPolicy::requiresDomainReview('high', 'platform'));
});
$t->test('Bundle freshness accepts exact current active source snapshots', function (): void {
    $hash = hash('sha256', 'v1');
    BundleFreshnessGuard::assertCurrent(
        array(array('key' => 'home.title', 'source_hash' => $hash, 'source_version' => 1, 'unit_uuid' => 'u1')),
        fn (string $key): ?array => 'home.title' === $key ? array('status' => 'active', 'source_hash' => $hash, 'source_version' => 1) : null
    );
});
$t->test('Bundle freshness rejects corrected or retired source snapshots', function (): void {
    $old = hash('sha256', 'v1');
    $new = hash('sha256', 'v2');
    TestHarness::assertThrows(
        fn () => BundleFreshnessGuard::assertCurrent(
            array(array('key' => 'home.title', 'source_hash' => $old, 'source_version' => 1, 'unit_uuid' => 'u1')),
            fn (string $key): ?array => array('status' => 'active', 'source_hash' => $new, 'source_version' => 2)
        ),
        InvalidArgumentException::class
    );
});
