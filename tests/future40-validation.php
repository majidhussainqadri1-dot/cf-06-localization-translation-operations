...
$t->test('OCR threshold must remain within 0..1', fn () => TestHarness::assertThrows(
    fn () => $s->evaluate('CF06-FUT-016', ['confidence'=>0.95,'threshold'=>-0.1]),
...
$t->test('Calendar layer rejects impossible Gregorian dates', fn () => TestHarness::assertThrows(
    fn () => $s->evaluate('CF06-FUT-020', ['canonical_iso_date'=>'2026-99-99','display_mode'=>'dual']),
    InvalidArgumentException::class
));
...
$t->test('Locale launch coverage and threshold remain bounded percentages', function () use ($s): void {
    TestHarness::assertThrows(fn () => $s->evaluate('CF06-FUT-026', ['coverage_percent'=>101,'critical_missing'=>0,'threshold_percent'=>95]), InvalidArgumentException::class);
    TestHarness::assertThrows(fn () => $s->evaluate('CF06-FUT-026', ['coverage_percent'=>99,'critical_missing'=>0,'threshold_percent'=>-1]), InvalidArgumentException::class);
});
