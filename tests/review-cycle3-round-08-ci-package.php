...
$t->test('CI verifies actual checkout identity before testing and packaging',function()use($ci):void{
    TestHarness::assertTrue(substr_count($ci,'git rev-parse HEAD')>=2);
    TestHarness::assertTrue(substr_count($ci,'EXPECTED_SHA: ${{ github.sha }}')>=2);
});
