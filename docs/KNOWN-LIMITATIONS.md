...
- Source-specific legacy-data backfill, dual-read/shadow comparison, reversible cutover and rollback against the actual staging data stores. The repository provides inventory/dry-run/checkpoint evidence primitives, but real backfill cannot be claimed without the concrete legacy owner contracts and staging data.
- Real File 00 identity/authorization, File 19 notification delivery, File 20 shell/language preference, Files 22/23 composer/dashboard, File 24 assurance, File 25 visual RTL/LTR, File 26 search/transliteration, CF-04 media, and native-domain contract acceptance.
