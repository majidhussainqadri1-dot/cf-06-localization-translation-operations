...
final class LocaleAccessibilityGuard
{
    ...
    private static function deviceMatrix(array $input): array
    {
        ...
        if (! is_array($requested)) {
            throw new InvalidArgumentException('viewports must be an array.');
        }
        $viewports = self::CORE_VIEWPORTS;
        foreach ($requested as $width) {
            if (filter_var($width, FILTER_VALIDATE_INT) === false) {
                throw new InvalidArgumentException('Viewport width must be an integer.');
            }
            $width = (int)$width;
            if ($width < 320 || $width > 2560) {
                throw new InvalidArgumentException('Viewport width must be within 320-2560 pixels.');
            }
            $viewports[] = $width;
        }
    ...
    private static function accessibility(array $input): array
    {
        ...
        if (null === $locale) {
            throw new InvalidArgumentException('target_locale must be a valid BCP47-style locale.');
        }
        foreach (['alt_text','captions','transcript','aria_labels'] as $field) {
            if (! is_array($input[$field] ?? null) || [] === array_filter($input[$field], static fn($v): bool => '' !== trim((string)$v))) {
                throw new InvalidArgumentException($field . ' must contain translated accessibility text.');
            }
        }
    ...
    private static function registerProfile(array $input): array
    {
        ...
        if (! in_array($policy, ['preserve-approved','source-exact','locale-approved-map'], true)) {
            throw new InvalidArgumentException('Unsupported honorific policy.');
        }
        $input['target_locale'] = $locale;
        $input['honorific_policy'] = $policy;
        return $input;
    }
    ...
    private static function numerals(array $input): array
    {
        ...
        if (null === $locale) {
            throw new InvalidArgumentException('locale must be a valid BCP47-style locale.');
        }
        $input['locale'] = $locale;
        return $input;
    }
    ...
    {
        $source = trim((string)($input['text'] ?? ''));
        if ('' === $source) {
            return $out;
        }
        $protected = '~(https?://[^\s<>)]+|\{[^{}]*\}|</?[^>]+>|%(?:\d+\$)?[sdfoxegc])~u';
        $parts = preg_split($protected, $source, -1, PREG_SPLIT_DELIM_CAPTURE);
        if (false === $parts) {
            return $out;
        }
        $rendered = '';
        foreach ($parts as $part) {
            if ('' === $part) { continue; }
            if (1 === preg_match($protected, $part)) {
                $rendered .= $part;
                continue;
            }
            $rendered .= preg_replace_callback('/[\p{L}]+/u', static function(array $m): string {
                ...
                return '[' . $word . str_repeat('~', max(1, (int)ceil($length * 0.35))) . ']';
            }, $part) ?? $part;
        }
        $out['result']['text'] = '⟦' . $rendered . '⟧';
        $out['result']['protected_tokens_preserved'] = true;
        $out['result']['special_character_probe'] = 'ÅßЖ中ع';
        $out['result']['rtl_probe_text'] = true === ($input['rtl_probe'] ?? false) ? "\u{2067}RTL عينة\u{2069}" : null;
    ...
    {
        $out['result']['target_locale'] = (string)$input['target_locale'];
        $out['result']['meaning_equivalence_human_review_required'] = true;
        $out['result']['screen_reader_acceptance_required'] = true;
        return $out;
    }
    ...
    private static function numeralResult(array $input, array $out): array
    {
        ...
        $targetSystem = (string)($input['system'] ?? 'latin');
        $out['result']['canonical_text'] = $source;
        $out['result']['locale'] = (string)$input['locale'];
        $out['result']['display_text'] = self::transformDisplayDigits($source, $targetSystem);
        $out['result']['text'] = $out['result']['display_text'];
        $out['result']['display_only'] = true;
        $out['result']['canonical_numeric_value_mutated'] = false;
        return $out;
    }
    ...
    private static function transformDisplayDigits(string $text, string $system): string
    {
        ...
        $parts = preg_split($protected, $text, -1, PREG_SPLIT_DELIM_CAPTURE);
        if (false === $parts) { return $text; }
        $output = '';
        foreach ($parts as $part) {
            if ('' === $part) { continue; }
            if (1 === preg_match($protected, $part)) { $output .= $part; continue; }
            $latin = strtr($part, array_combine($maps['arabic-indic'], $maps['latin']) + array_combine($maps['persian'], $maps['latin']));
            $output .= strtr($latin, array_combine($maps['latin'], $maps[$system]));
        }
        return $output;
    }
    ...
    private static function lineBreakResult(array $input, array $out): array
    {
        ...
        $parsed = LocaleValidator::parse($locale);
        $script = (string)($parsed['script'] ?? '');
        if ('' === $script) {
            $script = match ($parsed['language'] ?? '') {
                'ur','ar','fa','ps','sd','ug','dv','ku' => 'Arab',
                'he' => 'Hebr',
                default => 'Latn',
            };
        }
        $out['result']['locale'] = $locale;
        $out['result']['direction'] = LocaleValidator::direction($locale);
        $out['result']['script'] = $script;
        return $out;
    }
    ...
    {
        $out['result']['locale'] = (string)$input['locale'];
        $out['result']['direction'] = LocaleValidator::direction((string)$input['locale']);
        $out['result']['bidi_safety'] = 'validated';
        $out['result']['legacy_override_allowed'] = false;
        return $out;
    }
}
