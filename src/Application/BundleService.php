...
final class BundleService
{
    ...
    {
        $bundle=$this->repo->find('bundles',$uuid)??throw new InvalidArgumentException('Locale bundle not found.');if(in_array($to,array('active','rolled_back','superseded','invalidated'),true)){throw new InvalidArgumentException('Bundle activation, invalidation and rollback require dedicated controlled paths.');}StateMachine::assert('bundle',(string)$bundle['status'],$to);if(in_array($to,array('staged','canary'),true)&&empty($bundle['signature'])){throw new InvalidArgumentException('Signed locale bundle is required for release.');}
    ...
    public function rollback(string $activeUuid,string $targetUuid,int $activeVersion): array
    {
        ...
        $result=$this->withLocaleReleaseLock($locale,function()use($activeUuid,$targetUuid,$activeVersion,$locale):array{
            ...
            return $this->tx->run(function()use($wpdb,$active,$target,$activeVersion,$activeUnits,$targetUnits):array{$rolled=$this->repo->updateVersioned('bundles',(string)$active['uuid'],$activeVersion,array('status'=>'rolled_back'));if(!empty($activeUnits)){$ph=implode(',',array_fill(0,count($activeUnits),'%s'));$reverted=$wpdb->query($wpdb->prepare("UPDATE ".Database::table('units')." SET status='approved',released_bundle_uuid=NULL,row_version=row_version+1,updated_at=%s WHERE uuid IN ({$ph}) AND released_bundle_uuid=%s AND status='released'",Database::now(),...array_merge($activeUnits,array($active['uuid']))));if(false===$reverted||(int)$reverted!==count($activeUnits)){throw new RuntimeException('Active bundle units could not be reconciled exactly during rollback.');}}$newActive=$this->repo->updateVersioned('bundles',(string)$target['uuid'],(int)$target['row_version'],array('status'=>'active','activated_by'=>get_current_user_id(),'activated_at'=>Database::now()));$ph=implode(',',array_fill(0,count($targetUnits),'%s'));$activated=$wpdb->query($wpdb->prepare("UPDATE ".Database::table('units')." SET status='released',released_bundle_uuid=%s,row_version=row_version+1,updated_at=%s WHERE uuid IN ({$ph}) AND target_locale=%s AND status='approved' AND released_bundle_uuid IS NULL",$target['uuid'],Database::now(),...array_merge($targetUnits,array($target['locale_tag']))));if(false===$activated||(int)$activated!==count($targetUnits)){throw new RuntimeException('Rollback bundle units could not be restored exactly.');}$this->audit->record('bundle',(string)$active['uuid'],'locale_bundle_rolled_back','success',array('target'=>$target['uuid'],'locale'=>$active['locale_tag'],'restored_units'=>count($targetUnits)));$this->outbox->enqueue('LocaleBundleRolledBack','bundle',(string)$active['uuid'],array('locale'=>$active['locale_tag'],'from'=>$active['uuid'],'to'=>$target['uuid']));return array('rolled_back'=>$rolled,'active'=>$newActive);});
