...
final class PrivacyService
{
    ...
    public function exportUser(int $userId,int $page=1,int $perPage=100): array
    {
        ...
        $queries=[
            ...
            'comments_metadata'=>['SELECT uuid,unit_uuid,audience,status,created_at,updated_at FROM '.Database::table('comments').' WHERE author_id=%d ORDER BY id ASC LIMIT %d OFFSET %d',1],
            'feedback'=>['SELECT uuid,locale_tag,resource_key,route_path,category,severity,status,outcome_text,assigned_to,created_at,updated_at FROM '.Database::table('feedback').' WHERE reporter_id=%d OR assigned_to=%d ORDER BY id ASC LIMIT %d OFFSET %d',2],
