...
final class ReleaseApprovalService
{
    ...
    public function approve(string $bundleUuid,array $input): array
    {
        ...
        if(!in_array((string)$bundle['status'],array('approved','staged','canary'),true)){throw new InvalidArgumentException('Release approval requires an approved or staged bundle.');}
        ...
        if(!in_array($role,self::ROLES,true)||''===$evidenceRef||strlen($evidenceRef)>191||1!==preg_match('/^[a-f0-9]{64}$/D',$evidenceHash)||false===$stepUpTimestamp||abs(time()-$stepUpTimestamp)>900){throw new InvalidArgumentException('Release approval evidence or recent strict UTC step-up proof is invalid.');}
        ...
        foreach($existing as $approval){if((int)$approval['approver_id']===$actor||(string)$approval['approval_role']===$role){throw new InvalidArgumentException('Release approvals require distinct actors and distinct approval roles.');}}
    ...
    public function assertDualApproval(string $bundleUuid): void
    {
        ...
        foreach($rows as $row){$roles[(string)$row['approval_role']]=true;$actors[(int)$row['approver_id']]=true;}
        foreach(self::ROLES as $role){if(empty($roles[$role])){throw new InvalidArgumentException('Independent dual release approval is incomplete: '.$role);}}
        if(count($actors)<2){throw new InvalidArgumentException('Independent dual release approval requires two distinct actors.');}
    }
