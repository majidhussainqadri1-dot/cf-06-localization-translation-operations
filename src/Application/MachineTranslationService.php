...
final class MachineTranslationService
{
    ...
    {
        $job=$this->repo->find('vendor_jobs',$jobUuid)??throw new InvalidArgumentException('Vendor job not found.');$this->assertActiveProvider((string)$job['provider_key']);if(!in_array($job['status'],array('accepted','rejected','failed','human_reviewed'),true)){throw new InvalidArgumentException('Vendor job is not eligible for purge.');}
    ...
    private function assertActiveProvider(?string $expectedKey=null): array
    {
        ...
        $health=$this->provider->health();if(is_array($health)&&'disabled'===strtolower((string)($health['status']??''))){throw new InvalidArgumentException('Machine translation provider health is disabled.');}$healthRegion=is_array($health)?sanitize_text_field((string)($health['region']??'')):'';if(''!==$healthRegion&&!hash_equals((string)$row['region_code'],$healthRegion)){throw new InvalidArgumentException('Machine translation provider runtime region differs from the approved provider record.');}
        return $row;
    }
