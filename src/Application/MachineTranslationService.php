...
final class MachineTranslationService
{
    ...
    {
        $job=$this->repo->find('vendor_jobs',$jobUuid)??throw new InvalidArgumentException('Vendor job not found.');if('validated'!==(string)$job['status']){throw new InvalidArgumentException('Vendor job is not awaiting human review.');}if(!in_array($decision,array('accept','reject'),true)){throw new InvalidArgumentException('Vendor job review decision is invalid.');}$reason=sanitize_textarea_field($reason);if(strlen($reason)>2000){throw new InvalidArgumentException('Vendor job review reason exceeds the bounded limit.');}if('reject'===$decision&&''===trim($reason)){throw new InvalidArgumentException('Vendor job rejection reason is required.');}
